...
#if defined(V_WIRE_DYNAMI_MASK_PLANE) || defined(V_WIRE_DYNAMIC_MASK_SPHERE) || defined(V_WIRE_DYNAMIC_MASK_BOX)
...
half _V_WIRE_Size;
	...
	float3 _V_WIRE_DynamicMaskBoundsMin;
	float3 _V_WIRE_DynamicMaskBoundsMax;
	float4x4 _V_WIRE_DynamicMaskTRS;
#endif
...
inline half V_WIRE_DynamicMask(half3 maskPos)
{
	...
	#elif defined(V_WIRE_DYNAMIC_MASK_BOX)

		float3 vertex = mul(_V_WIRE_DynamicMaskTRS, float4(maskPos, 1)).xyz;
		
		half gValue = 0;
		if ((vertex.x > _V_WIRE_DynamicMaskBoundsMin.x && vertex.x < _V_WIRE_DynamicMaskBoundsMax.x) && (vertex.y > _V_WIRE_DynamicMaskBoundsMin.y && vertex.y < _V_WIRE_DynamicMaskBoundsMax.y) && (vertex.z > _V_WIRE_DynamicMaskBoundsMin.z && vertex.z < _V_WIRE_DynamicMaskBoundsMax.z))
			gValue = 1;

		return (_V_WIRE_DynamicMaskInvert > 0 ? gValue : 1 - gValue);
