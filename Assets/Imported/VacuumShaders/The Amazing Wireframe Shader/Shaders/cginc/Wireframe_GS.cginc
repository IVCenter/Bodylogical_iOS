...
void geom(triangle v2f_surf input[3], inout TriangleStream<v2f_surf> triStream)
{
		...
		input[0].worldPos = UnityObjectToClipPos(float4(input[0].worldPos, 1)).xyz;
		input[1].worldPos = UnityObjectToClipPos(float4(input[1].worldPos, 1)).xyz;
		input[2].worldPos = UnityObjectToClipPos(float4(input[2].worldPos, 1)).xyz;
	#else
