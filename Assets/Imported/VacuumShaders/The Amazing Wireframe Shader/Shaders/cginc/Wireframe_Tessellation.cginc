#ifndef VACUUM_WIREFRAME_TESSELLATION_CGINC
#define VACUUM_WIREFRAME_TESSELLATION_CGINC

#include "UnityCG.cginc"
#include "Tessellation.cginc"

float _V_WIRE_Tessellation;
#ifdef V_WIRE_TESSELLATION_DISTANCE_BASED
	float _V_WIRE_Tessellation_MinDistance;
	float _V_WIRE_Tessellation_MaxDistance;
#endif
#ifdef V_WIRE_TESSELLATION_EDGE_LENGTH
...
half2 _V_WIRE_Tessellation_DispTex_Scroll;
...
inline void TransformNormal(inout float4 vertex, inout float3 normal, float4 tangent, float2 texcoord)
{
	...
	float2 uv = texcoord * _V_WIRE_Tessellation_DispTex_ST.xy + _V_WIRE_Tessellation_DispTex_ST.zw + _V_WIRE_Tessellation_DispTex_Scroll * _Time.x;
	float d = tex2Dlod(_V_WIRE_Tessellation_DispTex, float4(uv, 0, 0)).r * _V_WIRE_Tessellation_DispStrength;
	v0 += normal * d;

	uv = saturate(texcoord + float2(_V_WIRE_Tessellation_DispTex_TexelSize.x, 0)) * _V_WIRE_Tessellation_DispTex_ST.xy + _V_WIRE_Tessellation_DispTex_ST.zw + _V_WIRE_Tessellation_DispTex_Scroll * _Time.x;
	d = tex2Dlod(_V_WIRE_Tessellation_DispTex, float4(uv, 0, 0)).r * _V_WIRE_Tessellation_DispStrength;
	v1 += normal * d;

	uv = saturate(texcoord + float2(0, _V_WIRE_Tessellation_DispTex_TexelSize.y)) * _V_WIRE_Tessellation_DispTex_ST.xy + _V_WIRE_Tessellation_DispTex_ST.zw + _V_WIRE_Tessellation_DispTex_Scroll * _Time.x;
	d = tex2Dlod(_V_WIRE_Tessellation_DispTex, float4(uv, 0, 0)).r * _V_WIRE_Tessellation_DispStrength;
	v2 += normal * d;
...
UnityTessellationFactors hsconst_surf (InputPatch<InternalTessInterp_appdata,3> v) {
  ...
  float4 tf;
...
#if defined(V_WIRE_TESSELLATION_DISTANCE_BASED) || defined(V_WIRE_TESSELLATION_EDGE_LENGTH)
	appdata_full vi[3];
	...
	#ifdef V_WIRE_TESSELLATION_DISTANCE_BASED
	  tf = UnityDistanceBasedTess(vi[0].vertex, vi[1].vertex, vi[2].vertex, _V_WIRE_Tessellation_MinDistance, _V_WIRE_Tessellation_MaxDistance, _V_WIRE_Tessellation);
	#else
...
v2f_surf ds_surf(UnityTessellationFactors tessFactors, const OutputPatch<InternalTessInterp_appdata, 3> vi, float3 bary : SV_DomainLocation) {
	...
	//Displace
#if defined(V_WIRE_TESSELLATION_NORMAL_RECONSTRUCT) && !defined(V_WIRE_ADDATIVE) && !defined(V_WIRE_MULTIPLY)
	TransformNormal(v.vertex, v.normal, v.tangent, v.texcoord.xy);
#else
	float2 uv = v.texcoord.xy * _V_WIRE_Tessellation_DispTex_ST.xy + _V_WIRE_Tessellation_DispTex_ST.zw + _V_WIRE_Tessellation_DispTex_Scroll * _Time.x;
	float d = tex2Dlod(_V_WIRE_Tessellation_DispTex, float4(uv, 0, 0)).r * _V_WIRE_Tessellation_DispStrength;
